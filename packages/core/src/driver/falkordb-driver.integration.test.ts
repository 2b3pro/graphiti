...
import { createFalkorClientAdapter, FalkorDriver } from './falkordb-driver';

const hasFalkorEnv =
  Boolean(process.env.FALKOR_HOST) ||
  Boolean(process.env.FALKOR_PORT) ||
  Boolean(process.env.FALKOR_URL);

const integrationTest = hasFalkorEnv ? test : test.skip;

describe('Falkor integration', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      const connectionConfig = {
        ...
        database,
        ...(process.env.FALKOR_URL ? { url: process.env.FALKOR_URL } : {})
      };
      const client = await Promise.race([
        createFalkorClientAdapter(connectionConfig),
        createTimeout(1500, 'Falkor connection timed out')
      ]);
      const driver = new FalkorDriver(connectionConfig, client);

      graphiti = new Graphiti({ driver });
      await Promise.race([
        graphiti.buildIndicesAndConstraints(),
        createTimeout(2000, 'Falkor setup timed out')
      ]);
      setupSucceeded = true;
    } catch {
      if (graphiti) {
  ...
  integrationTest('ingests a raw episode against Falkor', async () => {
    ...
    expect(Number(persisted.records[0]?.episode_count ?? 0)).toBe(1);
    expect(Number(persisted.records[0]?.entity_count ?? 0)).toBeGreaterThanOrEqual(2);
    expect(Number(persisted.records[0]?.edge_count ?? 0)).toBeGreaterThanOrEqual(1);
  });

  integrationTest('preserves temporal contradictions across batch ingest in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('reuses persisted alias metadata across episodes in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('uses relationship context to resolve ambiguous same-name entities in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('persists attribute history for changing model-style fields in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('persists timestamped company history in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('persists timestamped department history in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('persists timestamped location history in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('persists timestamped title history in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('persists timestamped status history in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('keeps the latest company when an older episode reports a prior employer in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('keeps the latest department when an older episode reports a prior team in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('keeps the latest location when an older episode reports a prior city in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('keeps the latest title when an older episode reports a prior title in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('keeps the latest status when an older episode reports a prior state in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('keeps the latest attribute value when an older episode reports a prior role in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('accumulates configured string-set attributes in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest('accumulates configured team string-set attributes in Falkor', async () => {
    if (!setupSucceeded || !graphiti) {
      return;
    }
  ...
  integrationTest(
    'keeps maintained attributes when older Falkor hydration returns null values',
    async () => {
      if (!setupSucceeded || !graphiti) {
        return;
      }
  ...
  integrationTest(
    'preserves multi-episode maintenance semantics across out-of-order batch ingest in Falkor',
    async () => {
      if (!setupSucceeded || !graphiti) {
        return;
      }
