...
The active Bun/TypeScript port lives under `packages/`:
- `packages/shared`
- `packages/core`
- `packages/server`
- `packages/mcp`
...
Port planning and handoff docs live under `spec/`, especially `spec/bun-typescript-port-plan.md` and `spec/typescript-port-prd.md`.
...
- `~/.bun/bin/bun test packages`: run the Bun/TypeScript test suite.
- `~/.bun/bin/bunx tsc -b --pretty false`: typecheck all TypeScript workspace packages.
- `rm -rf packages/*/dist packages/*/tsconfig.tsbuildinfo`: clear generated build artifacts before or after Bun test runs.

Recommended clean verification flow for the TypeScript port:
- `rm -rf packages/*/dist packages/*/tsconfig.tsbuildinfo`
- `~/.bun/bin/bun test packages`
- `~/.bun/bin/bunx tsc -b --pretty false`

In this environment, prefer the full Bun path (`~/.bun/bin/bun`) over bare `bun`.
...
For the TypeScript port, keep tests next to the package source under `packages/*/src/*.test.ts`. Clean `packages/*/dist` before running Bun tests when there is any chance generated `dist/*.test.js` files exist, otherwise Bun may double-run tests from source and build output.
